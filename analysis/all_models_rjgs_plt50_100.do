...
* Created by: rg
* Edited on: 31 March 25
* Edited by: rg
* Stata v.18.0

* does
	* runs all models using:
		* dataset including plots < 50 and plots < 100 
		...
		* model 2: loop running lasso for each rf product and saving /// 
		selected vars 
		* $geo global is also subject to lasso selection, not only rf metrics
...
* open dataset plots < 50
	use 		"$data/countries/aggregate/allrounds_final_weather_cp_plt50.dta", clear
	
* open dataset plots < 100
	*use 		"$data/countries/aggregate/allrounds_final_weather_cp_plt50.dta", clear
...
* collapse the data to a hh level 
	collapse 	(first) country survey admin_1* admin_2* admin_3* crop cluster_id hh_id_obs /// 
				...
				(first) v02_rf2 v04_rf2 v05_rf2 v07_rf2 v10_rf2 hh_asset_index farm_size ///
				(count) mi_* /// 
...
* drop Mali bc hh and managers cannot be tracked 
	...
	svyset 		ea_id_obs [pweight=wgt_adj_surveypop], strata(strata) singleunit(centered) /// 
				...
				urban plot_owned farm_size v02_rf2 v04_rf2 v05_rf2 v07_rf2 v10_rf2 /// 
				hh_asset_index d_* indc_*) vce(bootstrap)
...
* create weight adj
	...
	*** if plots < 100, then 2,140 obs dropped 
	*** if plots < 50, then 2,140 obs dropped
...
* survey design
	svyset 		ea_id_obs [pweight=wgt_adj_surveypop], strata(strata) singleunit(centered) /// 
				...
				irrigated intercropped crop_shock hh_shock hh_size /// 
				formal_education_manager female_manager age_manager hh_electricity_access /// 
				urban plot_owned v02_rf2 v04_rf2 v05_rf2 v07_rf2 v10_rf2 hh_asset_index /// 
				farm_size d_* indc_*) vce(bootstrap)
