...
* does
	* runs all models using:
		* same as main files
		* we use dataset 2: leave missing values as missing, only impute top 5%
...
* merge with hh 
	merge 		m:1 wave survey country hh_id_obs using ///
				"$export1/dta_files_merge/hh_included_labor_mss.dta"
...
***********************************************************************
**# d2 - decomposition: within vs between households (early vs late)
***********************************************************************

* NOTE:
* - Run this on the plot-level file BEFORE you collapse to hh/manager/cluster.
* - Uses area-weighted household productivity in each period.
* - Uses survey household weights (pw) for between-household composition.
* - Works even if plot IDs are not consistent across waves (because it aggregates within period).

* --- choose early/late bins (edit cutoffs if you prefer) ---
gen     period = .
...
* outcome used in regressions is ln_yield_cp = asinh(yield_cp)
* plot area = plot_area_GPS
* labor vars already scaled per ha earlier in your code (total_labor_days, hired, family)

* --- household-period total area (for within-household plot weights) ---
bysort  hh_id period: egen hh_area_pt = total(plot_area_GPS)
gen     w_plot_hh = plot_area_GPS / hh_area_pt

* --- household-period productivity (area-weighted within hh) ---
gen     y_plot_w = w_plot_hh * ln_yield_cp
bysort  hh_id period: egen y_hh_pt = total(y_plot_w)

* --- household-period labor intensity (area-weighted within hh) ---
* (these are already per-ha inputs in your pipeline; we still area-weight to be consistent)
gen     Ltot_plot_w = w_plot_hh * total_labor_days
gen     Lfam_plot_w = w_plot_hh * total_family_labor_days
gen     Lhir_plot_w = w_plot_hh * total_hired_labor_days

bysort  hh_id period: egen Ltot_hh_pt = total(Ltot_plot_w)
bysort  hh_id period: egen Lfam_hh_pt = total(Lfam_plot_w)
bysort  hh_id period: egen Lhir_hh_pt = total(Lhir_plot_w)

* keep one record per household-period for decomposition
preserve
keep    hh_id country period pw y_hh_pt Ltot_hh_pt Lfam_hh_pt Lhir_hh_pt
bysort  hh_id period: keep if _n == 1

* --- normalize household weights within each period (so composition sums to 1) ---
bysort  period: egen pw_sum = total(pw)
gen     w_hh = pw / pw_sum

* --- reshape wide for early/late ---
reshape wide y_hh_pt Ltot_hh_pt Lfam_hh_pt Lhir_hh_pt w_hh, i(hh_id) j(period)

* drop households that are missing either period (optional robustness)
* If you want a BALANCED decomposition only, uncomment the next line.
* drop if missing(y_hh_pt0) | missing(y_hh_pt1)

* compute overall mean in each period
...
* --- within/between decomposition for productivity ---
...
* --- repeat for labor series (optional but recommended) ---
foreach v in Ltot_hh_pt Lfam_hh_pt Lhir_hh_pt {
    gen `v'_bar = (`v'0 + `v'1)/2
    gen d_`v'   = (`v'1 - `v'0)
    gen within_`v'  = wbar * d_`v'
...
* --- store results in a tiny dataset for LaTeX table creation ---
tempfile decomp
keep hh_id country y_hh_pt0 y_hh_pt1 w_hh0 w_hh1 within_y between_y ///
     Ltot_hh_pt0 Ltot_hh_pt1 within_Ltot_hh_pt between_Ltot_hh_pt ///
     Lfam_hh_pt0 Lfam_hh_pt1 within_Lfam_hh_pt between_Lfam_hh_pt ///
     Lhir_hh_pt0 Lhir_hh_pt1 within_Lhir_hh_pt between_Lhir_hh_pt

save `decomp', replace

* also write a readable log block (results scalars)
di "------------------------------------------------------------"
di "Household-level decomposition (Early vs Late)"
di "Outcome: ln_yield_cp (asinh yield/ha)"
di "Overall mean early (Y0): " %9.4f Y0
di "Overall mean late  (Y1): " %9.4f Y1
di "Total change (dY):       " %9.4f dY
di "Within component:        " %9.4f WITHIN_Y
di "Between component:       " %9.4f BETWEEN_Y
di "Check (within+between):  " %9.4f (WITHIN_Y + BETWEEN_Y)
di "------------------------------------------------------------"

restore

* save decomposition dataset to your merge folder for later tabulation
cap mkdir "$export1/dta_files_merge"
save "$export1/dta_files_merge/decomp_hh_early_late.dta", replace	
...
* creating missing value indicators at plot level
	foreach 	var of varlist yield_cp harvest_value_cp total_family_labor_days /// 
				total_hired_labor_days seed_value_cp /// 
...
* collapse the data to a hh level 
	collapse 	(first) country survey admin_1* admin_2* admin_3* crop cluster_id hh_id_obs ///
				...
				(first) v03_rf2 v10_rf2 v12_rf2 v18_rf8 v22_rf8 v04_rf3 v06_rf3 v07_rf3 v10_rf3 /// 
				...
				total_hired_labor_days n_plot_area_GPS = plot_area_GPS, /// 
				by(hh_id wave)
						
		
* replace invalid observations with missing values and drop flag variables 
	foreach 	var of varlist yield_cp harvest_value_cp total_family_labor_days /// 
				total_hired_labor_days seed_value_cp fert_value_cp plot_area_GPS {
...
* save for merge 
*	save 		"$export1/dta_files_merge/hh_included_labor_mss.dta", replace
...
* drop Mali bc hh and managers cannot be tracked 
	...
	svyset 		ea_id_obs [pweight=pw], strata(strata) singleunit(centered) /// 
				...
				v03_rf2 v10_rf2 v12_rf2 v22_rf8 /// 
...
* collapse the data to a plot manager level 
	collapse 	(first) country survey admin_1* admin_2* admin_3* crop cluster_id /// 
				...
				ln_dist_popcenter ln_elevation farm_size nb_plots  ///
				soil_fertility_index d_* indc_* ///
				(sum) yield_cp harvest_value_cp seed_value_cp fert_value_cp /// 
...
* collapse the data to a hh level 
	collapse 	(first) country survey admin_1* admin_2* admin_3* crop cluster_id /// 
				...
				soil_fertility_index d_* indc_* ///
				(sum) yield_cp harvest_value_cp seed_value_cp fert_value_cp /// 
				total_family_labor_days total_labor_days /// 
				total_hired_labor_days /// 
				(sum) plot_area_GPS nb_seasonal_crop /// 
				(max) organic_fertilizer inorganic_fertilizer used_pesticides crop_shock /// 
				plot_owned irrigated /// 
...
* collapse the data to cluster level 
	collapse 	(first) country survey admin_1* admin_2* admin_3* crop  ea_id_obs aez /// 
				...
				(mean) v03_rf2 v10_rf2 v12_rf2 v18_rf8 v22_rf8 v04_rf3 v06_rf3 v07_rf3 v10_rf3 /// 
				v03_rf4 v04_rf4 v07_rf4 v10_rf4 ///
				(count) mi_* /// 
				(count) n_yield_cp = yield_cp n_harvest_value_cp = harvest_value_cp ///  
				n_seed_value_cp = seed_value_cp n_fert_value_cp = fert_value_cp /// 
...
* save for image 
	graph 		export 	"$export1/figures/coefficients_plot_labor_mss.pdf", as(pdf) replace
