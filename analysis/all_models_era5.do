...
*	reg 		ln_yield_cp $selbaseline_chirps , vce(cluster ea_id_obs)
	svy: 		reg ln_yield_cp $selbaseline_era5
...
* drop Mali bc hh and managers cannot be tracked 
	...
	svyset 		ea_id_obs [pweight=pw], strata(strata) singleunit(centered) /// 
				...
				v03_rf4 v04_rf4 v07_rf4 v10_rf4 v18_rf8 /// 
...
*	erase 		"$export1/tables/model4/yield.txt"
	...
	bs4rw, 		rw(bsw*)  : areg ln_yield_cp $selbaseline_4_5_6_era5 /// 
...
* create the graph
	...
	coefplot 	(model1, mcolor(navy) ciopts(color(navy) recast(rcap))), bylabel(Model 1) || /// 
				...
				ylab( 0.04 "4" 0.03 "3" 0.02 "2" 0.01 "1" 0 "0" -0.01 "-1" -0.02 "-2" /// 
				-0.03 "-3" -0.04 "-4" -0.05 "-5", labsize(small) grid) /// 
				ytitle(Annual productivity change (%)) vertical xsize(5)


* save for image 
	graph 		export 	"$export1/figures/coefficients_plot_era5.pdf", as(pdf) replace

		
* regression table 
	esttab 		model1 model2 model3 model4 model5 model6 /// 
				using "$export1/tables/reg_results_era5.tex", /// 
